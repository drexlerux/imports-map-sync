...
class S3Manager {
  constructor(type = 'root') {
    ...
    this.s3 = new AWS.S3();
    this.checkEnvs(process.env, ['APP_ALIAS', 'CONFIG_BUCKET', 'PORT', 'MODE', 'INDEX', 'LOCAL_ORIGIN?']);
      ...
      checker(prefix, (exists) => {
        ...
        promises.push(
          this.importMapsContent(newPrefix).then((data) => {
            ...
            return this.upload({ prefix, body: jsonParseData }).then(() => data);
          })
        );
      });
    });
   ...
   */
  importMapsPath() {
    const { origin, pathname } = new URL(this.s3.getSignedUrl('getObject', this.getParams({})).toString());
  ...
  upload({ body, contentType = 'application/json', prefix = this.BRANCH }) {
    const options = { prefix, params: { Body: JSON.stringify(body, null, 2), ContentType: contentType } };
  ...
  getParams({ prefix = this.BRANCH, params = {} }) {
    ...
    const defaultParams = { Bucket: this.CONFIG_BUCKET, Key: importmapsPath.replace('{branch}', prefix) };
   ...
   * @returns {string}
   */
  inlineImportmaps(jsonRawData) {
    const regexPath = `/${this.appFolder()}/${this.BRANCH}`;
    return jsonRawData.replace(new RegExp(regexPath, 'g'), '').replace(new RegExp('\\.' + MICROTIME_ZERO, 'g'), '');
  }
